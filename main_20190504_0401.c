...
void Coloring(int order) // color배열에 색상값 넣기 
{
	...
	for(i = 0;i < 16;i++)
	{
		...
		for(height = 0;height < 8;height++)
		{
			...
			{
				color[(goomba[i].Y + height) * WIDTH + goomba[i].X + width - MapMovX] = (goomba_texture_high[height][width] != 11 ? goomba_texture_high[height][width] : array_high[goomba[i].Y + height][goomba[i].X + width]) | (goomba_texture_low[height][width] != 11 ? goomba_texture_low[height][width] : array_low[goomba[i].Y + height][goomba[i].X + width]) << 4;
			}
		}
	}
	for(height = 0;height < 16;height += 2)
	{
		for(width = 0;width < 12;width++)
		{
			...
			color[(height / 2 + mario.Y) * WIDTH + width + mario.X - MapMovX] = (mario_texture[height][width] != 16 && mario_texture[height][width] != 11 ? mario_texture[height][width] : array_high[height / 2 + mario.Y][width + mario.X]) | (mario_texture[height + 1][width] != 16 && mario_texture[height + 1][width] != 11 ? mario_texture[height + 1][width] : array_low[height / 2 + mario.Y][width + mario.X]) << 4;
		}
	}
}
