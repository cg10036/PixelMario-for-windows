...
struct Pos // mario -> no flag, goomba -> flag:goomba_move(x), QuestionMarkBox -> flag:enabled?
{
	...
	int Y;
	int flag;
} mario, goomba[27], Q[11]; // goomba_size : 16
...
void Coloring(int order) // color배열에 색상값 넣기 // color = 15 | 14 << 4 -> color >> 4 = 14 -> color ^ ((color >> 4) << 4) = 15
{
	...
	for(height = 0;height < HEIGHT;height++) // map
	{
	...
	for(i = 0;i < 27;i++) // goomba
	{
		if(!(goomba[i].X >= MapMovX - 16 && goomba[i].X <= WIDTH + MapMovX))
		{
			continue;
		}
		for(height = 0;height < 8;height++)
		{
			for(width = 0;width < 16;width++)
			{
				if(goomba[i].X + width - MapMovX < 0 || goomba[i].X + width - MapMovX >= WIDTH)
				{
					continue;
				}
	...
	for(i = 0;i < 11;i++) // Q
	{
		if(!(Q[i].X >= MapMovX - 16 && Q[i].X <= WIDTH + MapMovX))
		{
			continue;
		}
		for(height = 0;height < 8;height++)
		{
			for(width = 0;width < 16;width++)
			{
				if(Q[i].X + width - MapMovX < 0 || Q[i].X + width - MapMovX >= WIDTH)
				{
					continue;
				}
	...
	for(height = 0;height < 16;height += 2) // mario
	{
...
void goomba_move_thread(void *args) // 굼바 움직임 && 마리오&굼바 감지 
{
	...
	while(running)
	{
		...
		for(i = 0;i < 27;i++)
		{
			if(!(goomba[i].X >= MapMovX - 16 && goomba[i].X <= WIDTH + MapMovX - 128))
			{
				continue;
			}
			//마리오가 굼바 찌부시켰을때 :( 
...
void Input(void *args)
{
	...
	while(running)
	{
		...
		i = clock();
		if((GetAsyncKeyState(VK_UP) & 0x8000 || GetAsyncKeyState('W') & 0x8000 || GetAsyncKeyState(VK_SPACE) & 0x8000) && CanIJump() && tmp == -7)
		{
			tmp = 22;
			if(play_sound_selection)
			{
...
void play_sound(int num) // 원활한 노래 출력을 위해 2개 만듦 
{
	...
	{
		mciSendString("play goomba_Q.wav", NULL, 0, NULL);
	}
}
